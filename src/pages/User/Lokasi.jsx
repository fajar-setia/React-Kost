...
const LocationPage = () => {
  ...
  return (
    <div className="min-h-screen bg-stone-50 py-16">
      ...
      <div className="max-w-7xl mx-auto rounded-3xl overflow-hidden shadow-2xl border-4 border-white">
        <MapContainer
          center={center}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {locationsData.map(loc => (
            <div key={loc.id} className="bg-white p-8 rounded-2xl shadow-lg border border-stone-200 hover:shadow-xl transition-shadow duration-300">
              <h3 className="text-2xl font-bold text-stone-800 mb-2">{loc.name}</h3>
              <p className="text-stone-600 mb-4">{loc.address}</p>
              <div className="text-sm text-stone-500 italic">
                <span className="font-semibold">Koordinat:</span> {loc.latitude}, {loc.longitude}
              </div>
            </div>
          ))}
        </div>
